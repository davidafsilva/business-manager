...
public class SaleService implements ISaleService {
	...
	public File generateReport(User user, File fp, Sale sale) throws ApplicationException {
		...
		try {
			...
			final boolean isProductCodeIncluded = configs.get(ConfigurationKey.SALE_INFO_PROD_CODE.getCode()).getValue().equals("1");
			final boolean isProductDescriptionIncluded = configs.get(ConfigurationKey.SALE_INFO_PROD_DESCRIPTION.getCode()).getValue().equals("1");
			final boolean isProductQtyIncluded = configs.get(ConfigurationKey.SALE_INFO_PROD_QTY.getCode()).getValue().equals("1");
			final boolean isProductPriceIncluded = configs.get(ConfigurationKey.SALE_INFO_PROD_PRICE.getCode()).getValue().equals("1");
			final boolean isDiscountIncluded = configs.get(ConfigurationKey.SALE_INFO_DISCOUNT.getCode()).getValue().equals("1");
			final boolean isVATIncluded = configs.get(ConfigurationKey.SALE_INFO_VAT.getCode()).getValue().equals("1");
			final boolean isTotalsIncluded = configs.get(ConfigurationKey.SALE_INFO_TOTALS.getCode()).getValue().equals("1");
			...
			final Font sectionFont = new Font(Font.FontFamily.TIMES_ROMAN, 11, Font.BOLD | Font.UNDERLINE);
			final Font boldFont = new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.BOLD);
			final Font tableHeaderFont = new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.BOLD, BaseColor.WHITE);
			...
			header.add(new Paragraph("Frutogal - Ind\u00FAstria e Com\u00E9rcio de Produtos alimentares, Lda.", headerFont));
			...
			clientParagraph.add(new Paragraph("Informa\u00E7\u00E3o do cliente " + client.getCode() + ":", sectionFont));
			...
			if (sale.hasCardex()) {
				...
				try {
					clientParagraph.add(new Chunk(Image.getInstance(SaleService.class.getResource("/assets/check.png")), 0f, 0f));
				} catch (IOException e) {
			...
			saleHeaderParagraph.add(new Paragraph("Venda", new Font(Font.FontFamily.TIMES_ROMAN, 12, Font.BOLD | Font.UNDERLINE)));
			...
			saleHeaderTable.addCell(new Phrase(DateFormat.getDateInstance().format(sale.getSaleDate()), regularFont));
			...
			for (SaleProduct product : sale.getProducts()) {
				...
				if (isProductQtyIncluded) {
					...
					if (product.getAmount() % 1.0 == 0) {
						cell = new PdfPCell(new Phrase(String.format("%d%s", (int) product.getAmount(), unit), regularFont));
					} else {
						cell = new PdfPCell(new Phrase(String.format("%.2f%s", product.getAmount(), unit), regularFont));
					}
					cell.setHorizontalAlignment(Element.ALIGN_CENTER);
					saleTable.addCell(cell);
				}
				...
				if (isProductPriceIncluded) {
					if (product.getPrice() == null || product.getPrice() == 0) {
						...
						if (product.getPrice() % 1.0 == 0) {
							saleTable.addCell(new Phrase(String.format("%d \u20ac", (int) product.getPrice().doubleValue()), regularFont));
						} else {
							saleTable.addCell(new Phrase(String.format("%.2f \u20ac", product.getPrice()), regularFont));
				...
				if (isDiscountIncluded) {
					if (product.getDiscount() == null || product.getDiscount().trim().isEmpty()) {
						...
						try {
							...
							if (isDouble(product.getDiscount().trim()) || isInteger(product.getDiscount().trim())) {
								if (discount % 1.0 == 0) {
				...
				if (isVATIncluded) {
					if (product.getVat() == null) {
						...
						if (product.getVat() % 1.0 == 0) {
							cell = new PdfPCell(new Phrase(String.format("%d", (int) product.getVat().doubleValue()), regularFont));
						} else {
			...
			if (isTotalsIncluded) {
				...
				totalsTable.addCell(new Phrase(String.format("%.2f \u20ac", sale.getTotalBrute()), regularFont));
				
				cell = new PdfPCell(new Phrase("Total IVA", tableHeaderFont));
				cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				cell.setBackgroundColor(BaseColor.LIGHT_GRAY);
				totalsTable.addCell(cell);
				totalsTable.addCell(new Phrase(String.format("%.2f \u20ac", sale.getTotalVat()), regularFont));
				
				cell = new PdfPCell(new Phrase("Total desconto", tableHeaderFont));
				cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				cell.setBackgroundColor(BaseColor.LIGHT_GRAY);
				totalsTable.addCell(cell);
				totalsTable.addCell(new Phrase(String.format("%.2f \u20ac", sale.getTotalDiscount()), regularFont));
				
				cell = new PdfPCell(new Phrase("Total liquido", tableHeaderFont));
				cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				cell.setBackgroundColor(BaseColor.LIGHT_GRAY);
				totalsTable.addCell(cell);
				totalsTable.addCell(new Phrase(String.format("%.2f \u20ac", sale.getTotalLiquid()), regularFont));
	 ...
	 *        The value text
	 */
	private void addFormItem(Paragraph paragraph, Font labelFont, String label, Font valueFont, String value) {
	...
	static class Footer extends PdfPageEventHelper {
		...
		public void onEndPage(PdfWriter writer, Document document) {
			...
			final Font copyrightsFont = new Font(Font.FontFamily.TIMES_ROMAN, 6, Font.NORMAL, BaseColor.GRAY);
			...
			ColumnText.showTextAligned(writer.getDirectContent(),
					Element.ALIGN_CENTER, new Phrase(String.format("p\u00E1gina %d", writer.getPageNumber()), pageFont),
	...
	public void sendReport(User user, Sale sale, File fp) throws ApplicationException {
		...
		try {
			...
			author += " <bm@davidafsilva.pt>";
			sendEmail(author,
					configs.get(ConfigurationKey.EMAIL_ADDRESS.getCode()).getValue(), null, null,
					configs.get(ConfigurationKey.EMAIL_SUBJECT.getCode()).getValue(),
					configs.get(ConfigurationKey.EMAIL_BODY.getCode()).getValue(),
	 ...
	 * 
	 * @param from
	 *        The from address source of the email
	 *        The from will be ignored by the SMTP server if it's configured to
	 *        use the username (email) as source of the form parameter
	 * @param to
	 *        The destiny addresses of the email (comma separated for multiple addresses)
	 * @param cc
	 *        The CC addresses (comma separated for multiple addresses)
	 * @param bcc
	 *        The BCC addresses (comma separated for multiple addresses)
	 * @param subject
	 *        The email subject
	 * @param body
	 *        The email body (MIME)
	 ...
	 *         if any exception is thrown when sending the email
	 */
	private static void sendEmail(String from,
